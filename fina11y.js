...
socket.on('connect', function () {
    ...
    socket.on('testEmit', function (subCategory) {
        ...
        var accordionMainWrapper = $.parseHTML('<div class="accordion__item js-accordion-item active"><div class="accordion-header js-accordion-header" id="fina11y-error-category-id-8">Error List</div><div class="accordion-body js-accordion-body" style="display: block;"><div class="accordion-body__contents">inside main panel 1</div><div class="accordion js-accordion"></div></div></div>');

        // getting the current url 
        let currentUrl = window.location.href;
        // let currentUrl = "http://christuniversity.in/";

        // ajax call to fetch the data
        $.ajax({
            type: "GET", //rest Type
            url: "http://192.168.35.31:8005/aitoolfilter?url="+currentUrl+"&aicate="+subCategory,
            ...
            success: function (response) {
                ...
                $.each(response, function (index, errorObj) {
                    ...
                    // test to dynamically add box for error,remedy,remarks
                    if (index == 0) {
                        ...
                        $(accordionSubPanel).find("button[id=fina11y-error-submit-btn]").attr("id", "fina11y-error-submit-btn-" + errorObj.id);
                        // adding toolname to the button 
                        $(accordionSubPanel).find("button[id=fina11y-error-submit-btn-" + errorObj.id+"]").attr("data-toolname", errorObj.toolname);
...
if (document.readyState === 'complete') {
    ...
    link.href = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@1.1.3/finallycss.css";
...
function createfina11yBody() {
    ...
    a11yscript.src = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@1.1.3/finallyUi.js"

    // imported.src = '../a11yElementHighlighter/socket.io.min.js';
    imported.src = 'https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@1.1.3/socket.io.min.js';
...
$(document).on('click', '[id^=fina11y-error-submit-btn]', function (event) {
    ...
    $.ajax({
        url: "http://192.168.35.31:8005/remedystatus?id="+errorId+"&toolname="+toolName+"&url="+currentUrl,
        // dataType: "json",
        contentType: "application/json",
        method: "GET",
        // data: JSON.stringify(jsonData),
...
function findElementByClassAttrTextTagName(className, attributeList, elementText, tagName, errorCodeId) {
    ...
    if (className !== undefined) {
        ...
        // $('div:not([class])').not($("#moveableBody div"));
...
$(document).on('click', '#fina11y-fix-and-replace', () => {
    ...
    var accordion = $.parseHTML(' <div class="accordion js-accordion"></div>');
    var accordionMainWrapper = $.parseHTML('<div class="accordion__item js-accordion-item active"><div class="accordion-header js-accordion-header" id="fina11y-error-category-id-8">Replaced Elements</div><div class="accordion-body js-accordion-body" style="display: block;"><div class="accordion-body__contents">inside main panel 1</div><div class="accordion js-accordion"></div></div></div>');
