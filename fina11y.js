// var fina11yErrorDomElement = '<div class="tota11y tota11y-annotation-contrast tota11y-label tota11y-label-error"> <span class="tota11y-label-error-icon"> <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="100%" height="100%" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve"> <path d="M80.515,90.781H19.485c-6.145,0-10.779-2.168-13.052-6.103c-2.273-3.938-1.832-9.036,1.24-14.356l30.515-52.851 c3.07-5.321,7.266-8.251,11.811-8.251s8.741,2.93,11.811,8.251l30.515,52.851c3.072,5.32,3.513,10.418,1.24,14.356 C91.293,88.613,86.659,90.781,80.515,90.781z M50,12.384c-3.367,0-6.59,2.369-9.071,6.669L10.415,71.904 c-2.483,4.3-2.924,8.275-1.24,11.191c1.683,2.916,5.345,4.521,10.311,4.521h61.029c4.966,0,8.628-1.605,10.311-4.521 c1.683-2.916,1.243-6.89-1.24-11.191L59.071,19.053C56.59,14.753,53.367,12.384,50,12.384z M56.227,72.18 c0-3.172-2.572-5.744-5.744-5.744s-5.744,2.572-5.744,5.744c0,3.172,2.572,5.744,5.744,5.744S56.227,75.352,56.227,72.18z M56.382,37.613c0-3.257-2.641-5.898-5.898-5.898c-3.257,0-5.898,2.641-5.898,5.898l1.393,20.932h0.019 c0.202,2.312,2.121,4.132,4.486,4.132c2.187,0,4.012-1.551,4.434-3.613c0.034-0.167,0.037-0.345,0.052-0.518h0.04L56.382,37.613z"> </path> </svg> </span> <span class="tota11y-label-content"><div class="tota11y-label-text">Error</div><div class="tota11y-label-help"> (<a class="tota11y-label-link" href="#">?</a>)</div><div class="tota11y-label-detail"><p class="error-highlighter-db-error-name">This contrast is insufficient at this size.</p><textarea class="tota11y-error-suggest-txtarea" name="" id="" cols="30" rows="10" placeholder="Enter your suggestion here......"></textarea><hr> <br><button class="bot bot-red">Issue</button> <button class="bot bot-green">Not an Issue</button></div> </span></div>';
...
if (document.readyState === 'complete') {
    ...
    link.href = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y/finallycss.css";
...
function createfina11yBody() {
    ...
    a11yscript.src = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y/finallyUi.js";
...
function createFina11yErrorHighlightingElement(currentElement, parentLiTag) {
    ...
    // to remove all the previously generated error highlighting element and also the highlighting border
    ...
    if (currentElement.length > 0) {
        $.each(currentElement, function (index, element) {
            ...
            if ($("[id^=fina11y-not-an-issue-btn]").prop('disabled') === true) {
                console.log("error is alredy submitted");
...
function setErrorListFromDb() {
    ...
    $.ajax({
        type: "GET", //rest Type
        url: "http://192.168.35.31:8005/htmlcsinput?url=currentUrl+&page=1&size=100",
        ...
        success: function (response) {
            ...
            $.each(response.content, function (index, errorObj) {
                ...
                // var headingChild = $(errorElementDom).find(".fina11y-collapsible, .fina11y-collapsible-category").html(errorObj.testCase);
                ...
                // $(errorElementDom).find("button.fina11y-collapsible").attr('id', "tota11y-category-error-id-" + errorObj.id);
                // $(errorElementDom).find("a.tota11y-info-error-trigger").attr('id', "tota11y-category-error-id-" + errorObj.id);
                // $(errorCodeSnippet).attr('id', 'fina11y-error-code-id-' + errorObj.htmlcsobj.id);
                // $(errorCodeSnippet).attr('id', 'fina11y-error-code-id-' + errorObj.id)
                // removing the &lt char and replace it with < 
                // var codelt = errorObj.prbmSrc.replace(/\</g, '&lt;');
                // $(errorCodeSnippet).html(codelt);
        ...
        toggle: function ($this) {
            ...
            if (settings.oneOpen && $this[0] != $this.closest('.js-accordion').find('> .js-accordion-item.active > .js-accordion-header')[0]) {
                ...
                if (($this.attr('id').match('^fina11y-error-category-id-'))) {
                    ...
                    // clearing the highlighting elements
                    $('[id^=fina11y-error-highlighting-main-div-]').remove();
                    $("*").removeClass("tota11y-add-bg-boder");
                    //clearing the html cotent inside the tool to display new content.

                    $this.parent().find(".accordion-body.js-accordion-body").find(".accordion.js-accordion").empty();
                    // $(".tota11y-info-errors").empty();
                    $.ajax({
                        url: "http://192.168.35.31:8005/htmlcsoutputbyid?page=1&size=1000",
                        dataType: "json",
                        contentType: "application/json",
                        method: "POST",
                        data: JSON.stringify(jsonData),
                        success: function (response) {
                            ...
                            $.each(response.content, function (index, errorObj) {
                                ...
                                errorCount += 1;
                                // creating dynamic element that contains the error details and  appending it with the toolbody
                                // var errorElementDom = $.parseHTML('<div class="accordion__item js-accordion-item"> <div class="accordion-header js-accordion-header">Sub Panel 1</div><div class="accordion-body js-accordion-body"> <div class="accordion-body__contents"> inside main panel 1 sub panel 1 </div></div></div>');
                                // var errorElementDom = $.parseHTML('<div class="accordion__item js-accordion-item"> <div class="accordion-header js-accordion-header">Sub Panel 1</div><div class="accordion-body js-accordion-body"> <div class="accordion-body__contents"> <div class="fina11y-code-editor"> <span class="control"></span> <span class="control"></span> <span class="control"></span> <pre class="line-numbers"> <code class="fina11y-error-code-snippet"></code></pre> </div></div></div></div>');
                                var errorElementDom = $.parseHTML('<div class="accordion__item js-accordion-item"> <div class="accordion-header js-accordion-header">Sub Panel 1</div><div class="accordion-body js-accordion-body"> <div class="accordion-body__contents"> <div class="fina11y-code-editor"> <pre class="language-markup fina11y-error-code-snippet" contenteditable="true"> </pre> <button id="fina11y-error-submit-btn" class="fina11y-bot fina11y-bot-red">Issue</button> <button id="fina11y-not-an-issue-btn" class="fina11y-bot fina11y-bot-green">Not an Issue</button> </div></div></div></div>');

                                // adding error heading to fina11y body
                                // var headingChild = $(errorElementDom).find(".accordion-header");
                                $(errorElementDom).find(".accordion-header").html(errorObj.testCase);
                                // $(headingChild).append(errorObj.testCase);
                                // $(".tota11y-label-detail").html(errorObj.testCase);

                                //adding error code snippet into fina11y body

                                $(errorElementDom).find(".fina11y-error-code-snippet").text(errorObj.prbmSrc);
                                var $errorCodeId = 'fina11y-error-code-id-' + errorObj.id;
                                // console.log('error idd = ' + $errorCodeId);

                                $(errorElementDom).find(".fina11y-error-code-snippet").attr('id', 'fina11y-error-code-id-' + errorObj.id);
                                // console.log($(errorObj.prbmSrc1));
                                $(errorElementDom).attr('id', 'fina11y-error-code-snippet-id-' + errorObj.id);
                                // var errorCodeSnippet = $(errorElementDom).find('code');

                                // appending error id with the code tag.
                                $(errorElementDom).find(".accordion-header").attr('id', "fina11y-sub-category-error-id-" + errorObj.id);
                                // $(errorCodeSnippet).attr('id', 'fina11y-error-code-id-' + errorObj.htmlcsobj.id);
                                // $(errorCodeSnippet).attr('id', 'fina11y-error-code-id-' + errorObj.id)
                                // removing the &lt char and replace it with < 
                                // var codelt = errorObj.prbmSrc.replace(/\</g, '&lt;');
                                ...
                                $this.parent().find(".accordion-body.js-accordion-body").find(".accordion.js-accordion").append(errorElementDom);
                                findElementByCode(errorObj.prbmSrc, $errorCodeId);
                            });
                            //appending error count text to the tools main body.
                            $(".tota11y-info-error-count").text(errorCount);
                        },
            ...
            $(".tota11y-info-error-count").text(prevErrorCount);
...
function findElementByCode(domElement, $errorCodeId) {
    ...
    console.log($errorElemenInnerHtml);
    console.log("-----------------------------------------")
    ...
    if ($errorElementId !== undefined) {
        ...
        console.log("element id and class name are undefined !!!!");
        if($errorElementAttributesList !== undefined ){
            console.log($errorElementAttributesList);
            console.log($errorElemenInnerHtml);
        }
    }
...
function highlightElementWithId(elementId, $errorCodeId) {
    ...
    var elementPostion = $($errorElement).offset();
    createErrorHighlightingElementOverDom($errorElement,elementPostion, $errorCodeId)
    // console.log("element found");
}


//generate error highlighting element over the error element
function createErrorHighlightingElementOverDom($errorElement, elementPostion, $errorCodeId) {


    $($errorElement).addClass("tota11y-add-bg-boder");

    console.log($errorCodeId);
    var $elementPosition = elementPostion;
    var $errorHighlighterElement = $(fina11yErrorHighlightingElementAll);
    ...
    // console.log($parentErrorElement);
...
function findElementByClassAttrTextTagName(className, attributeList, elementText, tagName, errorCodeId) {
    ...
    if($finalErrorElement.length > 1){
        ...
        console.log("position before active:");
            ...
            if(carouselFound.length > 0){
               ...
               $(carouselItem).removeClass("active");
                ...
                $(carouselItem).addClass("active"); 
                 ...
                 console.log("position after active");
                 console.log($elementPosition);
