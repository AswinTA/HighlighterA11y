...
socket.on('connect', function () {
    ...
    socket.on('testEmit', function (subCategory) {
        ...
        if (subCategory !== "") {
            ...
            let categoryArray = subCategory.split('__');
            subCategory = categoryArray[1];
            socketMainCat = categoryArray[0];
            socketSubCat = categoryArray[1];
            ...
            $(accordionMainWrapper).find('.accordion-body__contents').html(socketMainCat + " > " + socketSubCat);
            ...
            $.ajax({
                type: "GET", //rest Type
                url: "https://a11y.works/javaGet/aitoolfilter?url=" + currentUrl + "&aicate=" + subCategory1 + "&maincate="+ socketMainCat,
                ...
                success: function (response) {
                    ...
                    $.each(response, function (index, errorObj) {
                        ...
                        $(accordionSubPanel).find("button[id=fina11y-error-cant-handle-btn-]").attr("data-toolname", errorObj.toolname);
                        $(accordionSubPanel).find("button[id=fina11y-error-cant-handle-btn-]").attr("id", "fina11y-error-cant-handle-btn-" + errorObj.id);
                        ...
                        $(accordionSubPanel).find('.fina11y-label-text-test-case').html(errorObj.testCase);
                        ...
                        $(accordionSubPanel).find(".accordion-header").html(trimErrorCodeForHeading(errorObj.prbmSrc));
...
if (document.readyState === 'complete') {
    ...
    link.href = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@2.2.6/finallycss.css";
...
function createfina11yBody() {
    ...
    a11yscript.src = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@2.2.6/finallyUi.js"
...
$(document).on('click', '[id^=fina11y-error-submit-btn]', function (event) {
    ...
    if (confirm('Do You Want to save the remedy ?')) {
        var currentBtn = $(this);
        ...
        // disabling the can't handle button
        $(this).parent().find("button[id^='fina11y-error-cant-handle-btn-']").attr("disabled", true);
        $(this).parent().find("button[id^='fina11y-error-cant-handle-btn-']").css('cursor', 'not-allowed');
        ...
        let testCase = $(this).parent().find(".fina11y-label-text-test-case").text();
        // fetching the current url
        let currentUrl = window.location.href;
        // let currentUrl = "https://www.typescriptlang.org/";

        // removing the classname / border created by this tool from the remedy
        try {
            errorSolution = remedyClassNameRemover(errorSolution)
            $(this).parent().find(".language-markup.fina11y-error-code-snippet").text(errorSolution);
        } catch (exception) {
            console.log(` ${exception} occured during removing the tota11y-add-bg-border classname`)
        }
        ...
        jsonData.mainCategory = socketMainCat;
        jsonData.subCategory = socketSubCat;
        jsonData.testCase = testCase;
        ...
        $.ajax({
            url: "https://a11y.works/javaGet/remedystatus?id=" + errorId + "&toolname=" + toolName + "&url=" + currentUrl +"&status=completed",
            contentType: "application/json",
            method: "POST",
            success: function (response) {
...
// cant handle button 
$(document).on('click', 'button[id^="fina11y-error-cant-handle-btn-"]', (evt) => {
    if (confirm("Do You Want Mark this error as CAN'T HANDLE ?")) {
        var errorId = $(evt.target).attr('id').replace("fina11y-error-cant-handle-btn-", "");
        // getting the toolname from button
        var currentButtonId = $(evt.target).attr('id');
        ...
        // disabling the can't handle button
        $(evt.target).parent().find("button[id^='fina11y-error-submit-btn-']").attr("disabled", true);
        $(evt.target).parent().find("button[id^='fina11y-error-submit-btn-']").css('cursor', 'not-allowed');

        let toolName = $("#" + currentButtonId).attr("data-toolname");
        let currentUrl = window.location.href;

        $.ajax({
            url: "https://a11y.works/javaGet/remedystatus?id=" + errorId + "&toolname=" + toolName + "&url=" + currentUrl +"&status=cant handle",
            contentType: "application/json",
            method: "POST",
            success: function (response) {
                console.log(response)
        ...
        init: function ($settings) {
            $(document).on('click', '[id^=fina11y-sub-category-error-id-]', function (evt) {
                ...
                if (parentAccordionText === "Submitted Remedies") {
                    ...
                    // auto scroll accordion to the top of the div
    ...
    if (className !== undefined) {
        ...
        if ($finalErrorElement.length !== 0) {
            if ($finalErrorElement.length > 1) {
                ...
                if (a11yErrorElement === undefined) {
                    console.log("test for handle multiple element")
                    $.each($finalErrorElement, (i, element) => {
                        if ((element.attributes.length) === attributeList.length) {
                            if (element.innerHTML.replace(/\s/g, '') === elementText.replace(/\s/g, '')) {
                                ...
                                createErrorHighlightingElementOverDom(element, elementPosition, errorCodeId);
...
$(document).on('click', '#fina11y-fix-and-replace', () => {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (index, errorObj) {
                var accordionSubPanel = $.parseHTML('<div class="accordion__item js-accordion-item active" id="fina11y-error-code-snippet-id-36"> <div class="accordion-header js-accordion-header" id="fina11y-sub-category-error-id-36"></div><div class="accordion-body js-accordion-body" style="display: none; height: 103px; padding-top: 10px; margin-top: 0px; padding-bottom: 10px; margin-bottom: 0px;"> <div class="accordion-body__contents"> <div class="fina11y-code-editor"> <div> <label class="tota11y-error-label">SC :</label> <span class="fina11y-label-text">..</span></div><div> <label class="tota11y-error-label">Test Case :</label> <span class="fina11y-label-text-test-case">..</span></div><div> <label class="tota11y-error-label">Category :</label> <span class="fina11y-label-text-category">..</span></div><div> <label class="tota11y-error-label-subcategory">Sub-category :</label> <span class="fina11y-label-text-subcategory">..</span></div><div> <label class="tota11y-error-label">ERROR CODE :</label> <pre class="fina11y-error-src-code"></pre> </div><label>Error Remedy</label> <pre class="language-markup fina11y-error-code-remedy" contenteditable="true" id="fina11y-error-code-id-36"> prblm source code here</pre> <label style="display:none"> Remarks </label> <pre class="language-markup fina11y-error-remedy-box" contenteditable="true" id="fina11y-error-remarks-id-36" style="display:none"> enter your remarks here</pre> <button id="fina11y-error-re-submit-btn" class="fina11y-bot fina11y-bot-red" style="display:none" >Re-Submit</button> <button id="fina11y-not-an-issue-btn" class="fina11y-bot fina11y-bot-green" style="display:none">Preview Change</button> </div></div></div></div>');
                ...
                $(accordionSubPanel).find(".fina11y-label-text-test-case").html(errorObj.testCase);
                $(accordionSubPanel).find(".fina11y-label-text-category").html(errorObj.mainCategory);
                $(accordionSubPanel).find(".tota11y-error-label-subcategory").html(errorObj.subCategory);
                $(accordionSubPanel).find(".accordion-header").html(trimErrorCodeForHeading(errorObj.error));
                $(accordionMainWrapper).find(".accordion.js-accordion").append(accordionSubPanel);
                ...
                try {
                    ...
                    console.log(`${exception} occured during replacing`);
                    $(accordionSubPanel).find('#fina11y-sub-category-error-id-' + errorObj._id).attr("style", "background-color:rgb(195, 44, 44) !important");
                }
...
$(document).on('click', "#fina11y-add-error-btn", (event) => {
    ...
    $(".tota11y-info-errors").append(accordion);
    // Auto fill category and subcategory using socket.
    if (socketMainCat !== undefined && socketSubCat !== undefined) {
        $('#fina11y-manual-error-cat').html(socketMainCat);
        $('#fina11y-manual-error-sub-cat').html(socketSubCat);
    }
});
...
$(document).on('click', "#fina11y-manual-error-submit-btn", (event) => {
    ...
    if (confirm('Do you want to save this error and remedy ?')) {
        ...
        if (emptyTextBox) {
            ...
            let errorTestCase = $('#fina11y-manual-error-test-case').text();
            ...
            jsonData.remark = errorRemarks;
            jsonData.mainCategory = errorCategory;
            jsonData.subCategory = errorSubCategory;
            jsonData.testCase = errorTestCase;
...
$(document).on('click', '#fina11y-view-remedy-btn', (event) => {
    ...
    var accordion = $.parseHTML(' <div class="accordion js-accordion"></div>');
    var accordionMainWrapper = $.parseHTML('<div class="accordion__item js-accordion-item active"><div class="accordion-header js-accordion-header" id="fina11y-error-category-id-8">Submitted Remedies</div><div class="accordion-body js-accordion-body" style="display: block;"><div class="accordion-body__contents"></div><div class="accordion js-accordion"></div></div></div>');
    // let currentUrl = "https://www.typescriptlang.org/";
    $(accordionMainWrapper).find('.accordion-body__contents').html(socketMainCat + " > " + socketSubCat);
    var jsonData = new Object();
    jsonData.urld = window.location.href;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // clearing the content in the tool main body  to avoid rendering duplicate content again and again.
            $('.tota11y-info-errors').empty();
            ...
            $.each(response, (i, element) => {
                var accordionSubPanel = $.parseHTML(' <div class="accordion__item js-accordion-item active" id="fina11y-error-code-snippet-id-36"> <div class="accordion-header js-accordion-header" id="fina11y-sub-category-error-id-36"></div><div class="accordion-body js-accordion-body" style="display: none; height: 103px; padding-top: 10px; margin-top: 0px; padding-bottom: 10px; margin-bottom: 0px;"> <div class="accordion-body__contents"> <div class="fina11y-code-editor"> <div> <label class="tota11y-error-label">SC :</label> <span class="fina11y-label-text">..</span></div><div> <label class="tota11y-error-label">Test Case : </label> <span class="fina11y-label-text-test-case">..</span></div><div> <label class="tota11y-error-label">ERROR CODE :</label> <pre class="fina11y-error-src-code"></pre> </div><label>Error Remedy</label> <pre class="language-markup fina11y-error-code-remedy" contenteditable="true" id="fina11y-error-code-id-36"> prblm source code here</pre> <label> Remarks </label> <pre class="language-markup fina11y-error-remedy-box" contenteditable="true" id="fina11y-error-remarks-id-36"> enter your remarks here</pre> <button id="fina11y-error-re-submit-btn" class="fina11y-bot fina11y-bot-red" >Re-Submit</button> <button id="fina11y-not-an-issue-btn" class="fina11y-bot fina11y-bot-green">Preview Change</button> </div></div></div></div>');
                ...
                $(accordionSubPanel).find(".accordion-header").html(trimErrorCodeForHeading(element.error));
                $(accordionSubPanel).find(".fina11y-label-text-test-case").html(element.testCase);
                $(accordionMainWrapper).find(".accordion.js-accordion").append(accordionSubPanel);
                remedyCount++;
            });
            $(accordion).append(accordionMainWrapper);
...
$(document).on('click', '[id^=fina11y-error-re-submit-btn-]', (evt) => {
    ...
    if (confirm('Do You want to re-submit this remedy?')) {
        ...
        let errorTestCase = $(parentDiv).find("span.fina11y-label-text-test-case").text();
        let currentBtn = $(evt.target);
        let currentButtonId = $(evt.target).attr('id');

        // removing the automatically generated class name from the remedy
        try {
            errorRemedy = remedyClassNameRemover(errorRemedy)
            $(evt.target).parent().find(".language-markup.fina11y-error-code-remedy").text(errorRemedy);
        } catch (exception) {
            console.log(` ${exception} occured during removing the tota11y-add-bg-border classname`)
        }
...
$(document).on('focus', 'pre[id^="fina11y-manual-error-"]', function (evt) {
    ...
    if ($(evt.target).html() === "Enter category") {
        ...
        $(evt.target).html("");
    } else if($(evt.target).html() === "Enter Test Case"){
        $(evt.target).html("");
    }
});
...
function manualAddErrorTxtBoxValidation() {
    ...
    let errorTestCase = $('#fina11y-manual-error-test-case').html();
    ...
    }else if(errorTestCase === "" || errorTestCase === "Enter Test Case"){
        $('#valid-error-txt').attr('style', 'display:block !important');
        $('#valid-error-txt').html("Enter a valid Test Case");
        return false;
    } else {
        return true;
    }
...
function trimErrorCodeForHeading(errorCode) {
    let charLimit = 50;
    errorCode = errorCode.substring(0, charLimit);
    var map = {
        //         '&': '&amp;',
        '<': '&lt;',
        '>': '&gt;'
    ...
    return errorCode.replace(/[<>]/g, function (m) { return map[m]; }) + " ...";
}

$(document).on('click', '#fina11y-manual-error-replace-btn', (evt) => {
    ...
    if (emptyTextBox) {
        let errorCode = $('#fina11y-manual-error-code').text();
        let errorRemedy = $.parseHTML($('#fina11y-manual-error-remedy').text());
        findElementByCode(errorCode, 'tota11y-manual-error');
        $(currentElement).replaceWith(errorRemedy);
    }
});

// place holder remove on focus remarks txtbox
$(document).on('focus', 'pre[id^="fina11y-error-remarks-id-"]', (evt) => {
    if ($(evt.target).text() === " enter your remarks here") {
        $(evt.target).text("");
    }
});

// function to remove automatically generated classname from the remedy. 
function remedyClassNameRemover(remedy) {
    let remedyDom = $.parseHTML(remedy);
    let listOfClass = remedyDom[0].classList;
    if (listOfClass.length !== 0 && listOfClass.length === 1) {
        remedyDom = $(remedyDom).removeAttr("class");
        var withoutClass = $(remedyDom)[0].outerHTML;
        return withoutClass;
    } else {
        remedyDom = $(remedyDom).removeClass("tota11y-add-bg-boder");
        var withoutClass = $(remedyDom)[0].outerHTML;
        return withoutClass;
    }
}