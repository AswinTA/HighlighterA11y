...
if (document.readyState === 'complete') {
    ...
    link.href = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@1.2.5/finallycss.css";
...
function createfina11yBody() {
    ...
    a11yscript.src = "https://cdn.jsdelivr.net/gh/AswinTA/HighlighterA11y@1.2.5/finallyUi.js"
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function (index, errorObj) {
                ...
                // $(accordionSubPanel).find("button[id=fina11y-error-submit-btn]").attr("id", "fina11y-error-submit-btn-" + errorObj._id);
...
$(document).on('click', "#fina11y-manual-error-submit-btn", (event) => {
    ...
    let errorRemarks = $('#fina11y-manual-error-remedy').text();
...
function findElementByIncompleteCode(errorCode, errorCodeId) {
    ...
    if (incompleteEle) {
        ...
        if (errorCode.indexOf('id=') != -1) {
            ...
            if (elementWithId) {
                console.log("element found with iddd")
        ...
        if (errorCode.indexOf('class=') != -1) {
            ...
            console.log("start index of classname ; " + errorCode.indexOf('class='));
            let classname = errorCode.substring(errorCode.indexOf("class="), errorCode.length);
            classname = classname.substring(0, errorCode.indexOf('" ') - 1);
            console.log(classname)

            classname = classname.substring(classname.indexOf('="') + 2, classname.length - 2);
            // search element with the classname
            let elementByClassname = $(classname);
            if (elementByClassname.length > 0 && elementByClassname.length === 1) {
                ...
                if (errorCode.indexOf('>') !== -1) {
                    ...
                    console.log("element with closetag");
                    console.log(attrArray)

                } else {
                    console.log("element witout closing tag")
